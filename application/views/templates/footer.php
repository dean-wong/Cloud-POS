<!-- Begin footer content -->

<footer class="footer">
    <div class="container">
        <p class="text-muted">&copy; 2015 Lensow. All rights reserved.</p>
    </div>
</footer>


<!-- Bootstrap core JavaScript
================================================== -->
<!-- Placed at the end of the document so the pages load faster -->
<!-- jQuery文件。务必在bootstrap.min.js 之前引入 -->
<!-- <script src="//cdn.bootcss.com/jquery/2.1.4/jquery.min.js"></script> -->
<script src="<?= base_url('assets/js/jquery.min.js') ?>"></script>

<!-- <script src="//cdn.bootcss.com/bootstrap/3.3.5/js/bootstrap.min.js"></script> -->
<script src="<?= base_url('assets/js/bootstrap.min.js') ?>"></script>

<!-- Jquery UI -->
<!-- <script src="//cdn.bootcss.com/jquery-ui-bootstrap/0.5pre/assets/js/jquery-ui-1.10.0.custom.min.js"></script> -->
<script src="<?= base_url('assets/js/jquery-ui-1.10.0.custom.min.js') ?>"></script>

<!-- <script src="//cdn.bootcss.com/lodash-compat/3.10.1/lodash.min.js"></script> -->
<script src="<?= base_url('assets/js/lodash.min.js') ?>"></script>

<!-- IE10 viewport hack for Surface/desktop Windows 8 bug -->
<script src="<?= base_url('assets/js/ie10-viewport-bug-workaround.js') ?>"></script>

<!-- -->
<!-- <script src="<?= base_url('assets/js/icheck.min.js') ?>"></script> -->

<script src="<?= base_url('assets/js/cover.js') ?>"></script>

</body>
</html>
